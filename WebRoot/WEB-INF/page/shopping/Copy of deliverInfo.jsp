  ...
  <TABLE cellSpacing=1 cellPadding=1 width="100%" bgColor=#eeeecc 
      ...
      <TABLE cellSpacing=1 cellPadding=4 width="100%" border=0 style="background:#F3F3F3 none repeat scroll 0 0;">
		...
		<TR> <TD><DIV align="right">性别：</DIV> </td>
          <td>  <input type="radio" name="gender" checked="<c:if test="${empty deliverInfo|| deliverInfo.gender=='MAN'}">checked</c:if>" value="MAN"/>先生
         <input type="radio" name="gender" checked="<c:if test="${deliverInfo.gender=='WOMAN'}">checked</c:if>" value="WOMAN"/>女士</td></TR>
