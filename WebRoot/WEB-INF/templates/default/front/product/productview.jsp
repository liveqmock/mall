...
<link rel="canonical" href="<%=WebProperty.readUrl("site.url") %>${product.shtml_File_Name}"/>
</head>
...
<li><a href="<%=WebProperty.readUrl("site.url")%>"><%=WebProperty.readUrl("site.front.homepage.name")%></a></li>
 <c:set var="out" value=""/>
  ...
  <c:set var="out" value="<li><a href='${category.url}'>${category.name }</a></li>${out}"/>
   </c:forEach>
${out}<li><a href="${product.shtml_File_Name}" title="${product.name}">${product.name}</a></li>
</ul></div>
...
<div class="m rank">
<div class="mt"><h2>${position[0].name}销售排行</h2></div><div class="mc"><ul class="tab"><li class="curr">同类别</li><li class="">同价位</li><li class="">同品牌</li></ul>
<ul class="tabcon">
<c:forEach items="${hotsell}" var="pro">
<li>
<div class="p-img"><a href="${pro.shtml_File_Name}"><img width="50" height="50" src="${pro.tempImageUrl }"/></a></div>
<div class="p-name"><a title="${pro.name }" href="${pro.shtml_File_Name}">${pro.name }</a></div>
<div class="p-price">${pro.sellPrice }</div>
</li>
</c:forEach>
</ul>
