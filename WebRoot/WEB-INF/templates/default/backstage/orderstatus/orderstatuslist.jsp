...
<TITLE>订单状态列表</TITLE><link type="text/css" rel="stylesheet" href="css/product/list.css">
...
<c:forEach items="${status}" var="sta">
    <tr style="background:#6f8ac4;text-align:center">
      <td> <INPUT TYPE="checkbox" NAME="orderIds" value="${sta.id}">${sta.id}</td>
      <td>${sta.name}</td>
      <td>${sta.description}</td>
      <td>
    <a href="control/shopping/orderstatus/manage.do?method=edit&id=${sta.id}">编辑</a>
     <a href="control/shopping/orderstatus/manage?method=delete&id=${sta.id}">删除</a>
      </td>
	</tr>
</c:forEach>
    <!----------------------LOOP END------------------------------->
...
<a href="control/shopping/orderstatus/manage.do?method=addInput">添加</a>
</div></td>
    </tr>
</table>
  </body>
