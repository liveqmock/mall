			...
			if (a.queryObject) {
				...
				jQuery.each(arguments, function() {
					...
					jQuery.each(q.split(/[&;]/), function() {
						...
						var val = decodeURIComponent(encodeURIComponent(this
								.split('=')[1]));
						if (!key)
							return;
						if ($numbers) {
...
eval(function(p, a, c, k, e, r) {
	...
	return p
		...
		(
				...
				47,
				47,
				'||var|expires|if|cookie|document|typeof|path|domain|||||||||||||length|jQuery|null|number|toUTCString|60|else|secure|substring|function|undefined|new|Date|setTime|getTime|24|1000|encodeURIComponent|join|split|for|trim|decodeURIComponent|break|return'
						.split('|'), 0, {}))
...
Date.prototype.toString = Date.prototype.toString.overwrite(function(format) {
	...
	if (typeof (format) == "string") {
		...
		result = result.replace(/yy|YY/, this.getFullYear().toString().substr(
				2, 2));
		result = result.replace(/MM/,
				this.getMonth() >= 9 ? this.getMonth() + 1 : "0"
						+ (this.getMonth() + 1));
		result = result.replace(/M/, this.getMonth());
		result = result.replace(/dd|DD/, this.getDate() > 9 ? this.getDate()
				: "0" + this.getDate());
		result = result.replace(/d|D/, this.getDate());
		result = result.replace(/hh|HH/, this.getHours() > 9 ? this.getHours()
				: "0" + this.getHours());
		result = result.replace(/h|H/, this.getHours());
		result = result.replace(/mm/, this.getMinutes() > 9 ? this.getMinutes()
				: "0" + this.getMinutes());
		result = result.replace(/m/, this.getMinutes());
		result = result.replace(/ss|SS/, this.getSeconds() > 9 ? this
		...
		var defaults = {
			...
			error : function(x, s, m) {
				alert("Status: "
						...
						+ "\nMessage: "
						+ msJSON.parse(((x.responseText) ? x.responseText
								: "Unknown")).Message);
			}
		};
...
TrimPath.parseTemplate_etc.statementDef={
		...
		"for" : {
			...
			prefixFunc : function(stmtParts, state, tmplName, etc) {
				...
				return [
						...
						"if ((", listVar, ") != null) { ", "var ", iterVar,
						"_ct = 0;", "for (var ", iterVar, "_index in ",
						listVar, ") { ", iterVar, "_ct++;", "if (typeof(",
						listVar, "[", iterVar,
						"_index]) == 'function') {continue;}",
						"__LENGTH_STACK__[__LENGTH_STACK__.length - 1]++;",
						"var ", iterVar, " = ", listVar, "[", iterVar,
						"_index];" ].join("");
			}
		},
		"forelse" : {
	...
	var parse = function(body, tmplName, etc) {
			...
			while (begStmt >= 0) {
				...
				if (blockrx) {
					...
					if (blockMarkerEnd >= 0) {
						...
						var blockEnd = body.indexOf(blockMarker,
								blockMarkerEnd + 1);
						if (blockEnd >= 0) {
					...
					if (body.substring(begStmt + offset, begStmt + 10 + offset)
		...
		if (state.stack.length != 0)
			throw new etc.ParseError(tmplName, state.line,
					"unclosed, unmatched statement(s): "
							+ state.stack.join(","));
	...
	var emitSectionTextLine = function(line, funcText) {
		...
		while (endExprPrev + endMarkPrev.length < line.length) {
			...
			emitText(line.substring(endExprPrev + endMarkPrev.length, begExpr),
					funcText);
			var exprArr = line.substring(begExpr + begMark.length, endExpr)
					.replace(/\|\|/g, "#@@#").split('|');
	...
	TrimPath.processDOMTemplate = function(elementId, context, optFlags,
			optDocument, optEtc) {
		return TrimPath.parseDOMTemplate(elementId, optDocument, optEtc)
				.process(context, optFlags);
	}
})();
/* pagination */
				...
				function drawLinks() {
					...
					var appendItem = function(page_id, appendopts) {
						...
						if (page_id == current_page) {
							var lnk = $("<a href='javascript:void(0)' class='current'>"
									+ (appendopts.text) + "</a>");
						} else {
...
var category = {
	...
	template : [
			"<div class='{0}'>{1}</div>",
			"<span><h3>{0}</h3><s></s></span>",
			"<div class='i-mc'><div onclick=\"$(this).parent().parent().removeClass('hover')\" class=\"close\"></div><div class='subitem'>{0}</div><div id='JD_sort_{1}' class='fr'><div class='iloading'>正在加载中，请稍候...</div></div></div>",
			"<dl class='{0}'><dt>{1}</dt><dd>{2}</dd></dl>",
			"<em>{0}</em>",
			"<a href='{0}'>{1}</a>",
			"<div class='extra'><a href='http://www.360buy.com/allSort.aspx'>全部商品分类</a></div>",
			"<ul>{0}</ul>", "<li>·{0}</li>" ],
	...
	getDataService : function(t) {
		...
		for ( var w = 0; w < C.length; w++) {
			var g = (w == 0) ? "item fore" : "item", f = A(B[5], [ C[w].u,
					C[w].n ]), e = [], d = [], s = C[w].i, n, m;
			for ( var v = 0; v < s.length; v++) {
				n = s[v].i;
				var l = [], k = (s[v].u) ? A(B[5], [ o(s[v].u), s[v].n ])
						: s[v].n;
	...
	getBrandService : function(t) {
		...
		var h = t.data, f = this.getString, m = this.getUrl, r = this.template;
		for ( var q = 0; q < h.length; q++) {
			var e = h[q]["b"], d = h[q]["p"], g = h[q]["id"], i = [], p = [], o = [], l;
			if (e.length > 0) {
				for ( var n = 0; n < e.length; n++) {
					l = f(r[5], [ e[n]["u"], e[n]["n"] ]);
					p.push(f(r[4], [ l ]));
				}
				var j = (g == "k") ? "brands brands-book" : "brands", s = (g == "k") ? "推荐品牌出版商"
						: "推荐品牌";
	...
	$.fn.jdPosition = function(option) {
		...
		case "center":
			...
			$(this)
					.css(
							{
								left : $.browser.scroll().left
										+ Math
												.max(
														($.browser.client().width - ow) / 2,
														0) + "px",
								top : (!$.browser.isIE6) ? (h ? $.browser
										.scroll().top
										: ($.browser.scroll().top
												+ Math
														.max(
																($.browser
																		.client().height - oh) / 2,
																0) + "px"))
										: (($.browser.scroll().top <= $.browser
												.client().bodyHeight
												- oh) ? ($.browser.scroll().top
												+ Math
														.max(
																($.browser
																		.client().height - oh) / 2,
																0) + "px")
												: ($.browser.client().bodyHeight - oh)
														+ "px")
							});
			break;
		case "auto":
...
(function($) {
	$.fn.jdThickBox = function(option, callback) {
		...
		var close = function() {
			...
			if (s._autoReposi) {
				$(window).unbind("resize.jdThickBox").unbind(
						"scroll.jdThickBox")
			}
		};
		if (s.close) {
		...
		var init = function(element) {
			...
			inputData(element);
			if (s._autoReposi) {
				$(window).bind("resize.jdThickBox", reposi).bind(
						"scroll.jdThickBox", reposi)
			}
			;
			if (s._fastClose) {
...
var $callback = {
	...
	"input_event" : function(event) {
		...
		if ($GLOBAL_VAR._text == $dt._tbox.value || _e.keyCode == "40"
				|| _e.keyCode == "38") {
			$o.move(_e);
		} else {
			$GLOBAL_VAR._count = 0;
...
var Operator = function() {
	this.updatelist = function() {
		...
		for ( var _i = 0; _i < $GLOBAL_VAR._arr.length - 1; _i++) {
			...
			_html += $pro._htmls[0] + _t[0] + $pro._htmls[1] + (_i + 1)
					+ $pro._htmls[2] + _t[1] + $pro._htmls[3] + _t[0]
					+ $pro._htmls[4];
		}
		_html += $pro._htmls[5];
		_html += $pro._htmls[6];
	...
	this.xg = function() {
		...
		for ( var _i = 0; _i < $GLOBAL_VAR._arr.length - 1; _i++) {
			...
			_html += $pro._xghtmls[0] + _t[0] + $pro._xghtmls[1] + _t[0]
					+ $pro._xghtmls[2];
		}
		if (_html.length < 1) {
	...
	this.moveUp = function() {
		...
		if ($GLOBAL_VAR._count == $GLOBAL_VAR._num) {
			...
			$GLOBAL_VAR._out_select = $util
					.get("d_" + ($GLOBAL_VAR._count + 1));
		}
	...
	this.selectText = function(_d) {
		...
		$callback.hidden();
		window.location.href = "http://search.360buy.com/search?keyword="
				+ _d.title;
	};
...
var Properties = {
	...
	"_htmls" : new Array(
			...
			"</div></li>",
			"",
			"<li class='close'><a href='javascript:void(0)' onclick=\"$('#tie').hide()\">关闭</a></li>"),
	"_xghtmls" : new Array("<div><a href=\"Search?keyword=", "\">",
			"</a></div>")
};
var Dt = function(t, d) {
	this._div = d;
...
(function($) {
	$.jdCalcul = function(pids) {
		var arr = null;
		var pids = pids.join(",");
		var dataUrl = "http://www.360buy.com/limitBuy.aspx?callback=?&ids="
				+ pids;
		...
		var init = function(data) {
			...
			if (data == {} || data == "" || s.start == null || s.start == ""
					|| s.end == null || s.end == "") {
				return;
			}
			;
			...
			var createHtml = function() {
				...
				$
						.each(
								s.pros,
								function(i) {
									var id = s.pros[i].id, tp = s.pros[i].tp, zt = (s.pros[i].zt == 1) ? "<div class='pi9'></div>"
											: "<div class='pi10'></div>", mc = unescape(s.pros[i].mc), qg = s.pros[i].qg, zk = "("
											+ s.pros[i].zk + "折)";
									html1 += html.replace(/\{0\}/g, id)
											.replace("{1}", tp).replace("{2}",
													zt).replace("{3}", mc)
											.replace("{4}", qg).replace("{5}",
													zk).replace(/\{6\}/g, purl);
								});
				html1 += "</ul>";
			...
			var run = function() {
				if (ST > 0) {
					...
					if (ET > 0) {
						...
						S = (ET - H * 3600) % 60;
						s.clockid.html("剩余<b>" + H + "</b>小时<b>" + M
								+ "</b>分<b>" + S + "</b>秒");
						ET--;
					} else {
						s.clockid.html("抢购结束");
...
function mlazyload(option) {
	...
	var pageTop = function() {
		var d = document, y = (navigator.userAgent.toLowerCase().match(/iPad/i) == "ipad") ? window.pageYOffset
				: Math.max(d.documentElement.scrollTop, d.body.scrollTop);
...
var jdRecent = {
	...
	setData : function(result) {
		this.element.append("<li><div class='p-img'><a href='" + result.url
				+ "'><img src='" + result.img
				+ "' /></a></div><div class='p-name'><a href='" + result.url
				+ "'>" + decodeURIComponent(result.name) + "</a></div></li>");
	}
};
