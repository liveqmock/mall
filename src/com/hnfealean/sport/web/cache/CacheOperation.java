...
public class CacheOperation {
	...
	public Hashtable getCacacheMap(){
		return cacheMap;
	}
	/** */
	/**
	 ...
	 * @return
	 */
	public Object getCacheData(Object o, String methodName,
			Object[] parameters, long intervalTime, int maxVisitCount) {
		
		Class oc = AopUtils.getTargetClass(o);//o instanceof Class ? (Class) o : o.getClass();
		...
		if (parameters != null) {
			for (int i = 0; i < parameters.length; i++) {
				if (parameters[i] instanceof Object[]) {
					key.append("-").append(
							Arrays.toString((Object[]) parameters[i]));
				} else {
		...
		} else if (maxVisitCount > 0
				&& (maxVisitCount - cacheData.getCount()) <= 0) {// 访问次数超出,启动线程更新数据
	...
	private Object invoke(Object o, String methodName, Object[] parameters,
		...
		try {
			...
			if (parameters != null) {
				...
				for (int i = 0; i < parameters.length; i++) {
					...
					if (parameters[i] instanceof Class) {
						...
						//pcs[i] = AopUtils.getTargetClass(parameters[i]);
					}
				}
			}
			Class oc = AopUtils.getTargetClass(o);//o instanceof Class ? (Class) o : o.getClass();
			...
			//FrontProductManager mss =	(FrontProductManager)o;
		...
		try {
			...
			if(interfaces!=null&&interfaces.length>0){
				for(int j=0;j<interfaces.length;j++){
					Method method = interfaces[j].getDeclaredMethod(methodName, pcs);
	 ...
	 * @param key
	 */
	private void daemonInvoke(Object o, String methodName, Object[] parameters,
			String key) {
	...
	public class MethodInfo {
		...
		public String toString() {
			...
			if (parameters != null) {
				...
				for (int i = 0; i < parameters.length; i++) {
					if (parameters[i] instanceof Object[]) {
						str.append(Arrays.toString((Object[]) parameters[i]))
								.append(",");
					} else {
